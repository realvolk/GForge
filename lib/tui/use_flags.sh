...
gforge_configure_use_flags() {
    ...
    tui_msg_quick "USE Flags" "Type to search, Space to toggle, Enter to confirm."

    local selected
    selected=$(tui_multiselect "USE Flags" "Toggle flags:" "Search flags..." 0 0 "${use_labels[@]}") || true
